...
window.isMouseDown = false; // variable global peut accéder partous
...
canvas.addEventListener(
    'mousemove',
    evt => (mousePosition = getMousePos(canvas, evt)),
    false
);
...
const setup = function() {
    ...
    world.loadScene(null, tableauFleche, tableauKey);
};
...
function renderObjects() {
    for (let i = 0; i < listeActor.length; i++) listeActor[i].render();
}
