...
export default class Actor {
    ...
    update() {
        ...
        this.positionIso = this.mapPosToTilePos(
            this.position.x,
            this.position.y
        );

        // Mise a jour des composant
        if (this.animationSprite != undefined) this.animationSprite.update();
        if (this.collider != undefined) this.collider.update();
        for (let i = 0; i < this.behavior.length; i++)
            this.behavior[i].update();

        // Mise a jour de positionZ
        this.positionZ =
            this.positionIso.x + this.positionIso.y * this.diagonalMax;
    ...
    updateAfterCalcul() {
        ...
        if (this.collider != undefined) this.collider.updateAfterCalcul();
    ...
    render() {
        ...
        if (this.animationSprite != undefined) this.animationSprite.render();
        for (let i = 0; i < this.behavior.length; i++)
    ...
    getChilActor(name) {
        for (let i = 0; i < this.childrenActor.length; i++) {
            if (this.childrenActor[i].name == name)
                ...
                if (actorFound != undefined) return actorFound;
            }
        }
    }
    ...
    addAnimationSprite(
        ...
        this.animationSprite = new SpriteAnimation(
            this,
            src,
            nbFrame,
            nbAnimation,
