...
export default class MapRenderer extends BehaviorComposant {
    constructor() {
        ...
        for (let i = 0; i < this.tile_quantity; ++i) {
            ...
            if (i == 0) {
                ...
                img.src = 'assets/img/tiles/' + i + '.png';
            }
    ...
    update() {
        ...
        this.hoverTileX =
            Math.floor(mouse_y / tile_height + mouse_x / tile_width) - 1;
        this.hoverTileY = Math.floor(
            -mouse_x / tile_width + mouse_y / tile_height
        );
    }
    updateAfterCalcul() {
    ...
    renderTiles(x, y) {
        ...
        for (let tileX = 0; tileX < this.gridSize; ++tileX) {
            for (let tileY = 0; tileY < this.gridSize; ++tileY) {
                ...
                if (tile !== this.tiletype_empty)
                    this.renderTexturedTile(
                        this.tile_images[tile],
                        renderX,
                        renderY,
                        80
                    );
        ...
        if (
            this.hoverTileX >= 0 &&
            this.hoverTileY >= 0 &&
            this.hoverTileX < this.gridSize &&
            this.hoverTileY < this.gridSize
        ) {
            let renderX =
                x + (this.hoverTileX - this.hoverTileY) * tile_half_width;
            let renderY =
                y + (this.hoverTileX + this.hoverTileY) * tile_half_height;
