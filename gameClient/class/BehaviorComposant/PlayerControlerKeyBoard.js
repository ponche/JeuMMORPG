...
export default class PlayerControlerKeyBoard extends BehaviorComposant {
    ...
    update() {
        ...
        if (this.arrayKey[0]) direction.y -= 0.5 * this.actor.speed;
        if (this.arrayKey[1]) direction.y += 0.5 * this.actor.speed;
        if (this.arrayKey[2]) direction.x -= 1 * this.actor.speed;
        if (this.arrayKey[3]) direction.x += 1 * this.actor.speed;
