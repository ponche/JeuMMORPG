...
import {
    ArcadeBody,
    MapRenderer,
    PlayerControlerKeyBoard,
    PlayerLogDebuger,
    Teleporteur
} from './BehaviorComposant/index.js';
...
export default class Scene {
    ...
    loadScene(fileJson, tableauFleche, tableauKey) {
        ...
        bodyPlayer.animationSprite = new SpriteAnimation(
            bodyPlayer,
            'assets/img/sprites/characters/robot.png',
            9,
            8
        );
        ...
        bodyPlayer.addBehavior(
            new ArcadeBody([22, 23], bodyPlayer, this.listeActor)
        );
    ...
    getPrefab(fileJson) {
        ...
        xhr.open(
            'GET',
            'http://localhost/JeuMMORPG/gameClient/' + fileJson,
            false
        );
    ...
    getActor(name) {
        for (let i = 0; i < this.listeActor.length; i++) {
            if (listeActor[i].name == name) return listeActor[i];
            else {
